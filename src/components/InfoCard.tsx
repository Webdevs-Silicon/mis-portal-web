...
export default function InfoCard({ data, onBarSelect }: Props) {
  ...
  const normalizedBarValues = useMemo(() => {
    ...
    // Use absolute values for normalization to handle negative values properly
    const absValues = data.barValues!.map(Math.abs);
    const maxValue = Math.max(...absValues);
    // Avoid division by zero if all values are 0
    if (maxValue === 0) return data.barValues!.map(() => 0);

    // Scale all values to be between 0 and 1, relative to the max absolute value
    return data.barValues!.map((v) => Math.abs(v) / maxValue);
  }, [data.barValues]);
      ...
      {data.showBarGraph && data.barValues && normalizedBarValues.length > 0 ? (
        ...
        >
          ...
          {normalizedBarValues.map((normalizedHeight, i) => {
            const isSelected = i === displayIndex;
            const originalValue = data.barValues![i]; // Get the original value for the callback
            const isNegative = originalValue < 0; // Check if value is negative
            
            // Determine bar color based on value positivity
            let barColor;
            if (isNegative) {
              // Negative values: use red (#EF4444)
              barColor = isSelected ? "#EF4444" : "rgba(239, 68, 68, 0.5)";
            } else {
              // Positive values: use the original accent colors
              barColor = isSelected
                ? data.primaryAccentColor
                : data.secondaryAccentColor;
            }
