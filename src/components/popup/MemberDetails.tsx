...
const MemberDetails: React.FC<MemberDetailsProps> = ({ onClose, open }) => {
  ...
  // =============================
  // Map API Data to UI - UPDATED for new structure
  // =============================
  const mapApiData = (
    memberRes: GetMemberResponse,
    classRes: GetMemberClassificationsResponse
  ): ClassData[] => {
    // Get all class entries from MemberDetails API (filter out 'Main')
    const memberEntries = Object.entries(memberRes.MemberDetails)
      .filter(([key]) => key !== 'Main')
      .sort(([keyA], [keyB]) => parseInt(keyA) - parseInt(keyB)); // Sort by numeric key
    
    // Get all class entries from MemberClassifications API (filter out 'Main')
    const classEntries = Object.entries(classRes.MemberDetails)
      .filter(([key]) => key !== 'Main')
      .sort(([keyA], [keyB]) => parseInt(keyA) - parseInt(keyB)); // Sort by numeric key

    console.log("🟩 Member Details entries:", memberEntries);
    console.log("🟨 MemberClassifications entries:", classEntries);

    // Map class data by index (assuming same order in both responses)
    return classEntries.map(([key, classData], idx) => {
      const classInfo = classData as MemberClassData;
      const color = COLORS[idx % COLORS.length];
      
      // Get corresponding member data by index
      const memberEntry = memberEntries[idx];
      const memberCount = memberEntry 
        ? (memberEntry[1] as any).MemberCount?.toLocaleString() ?? '-'
        : '-';
      
      const shareBal = memberEntry 
        ? (memberEntry[1] as any).ShareBal ?? 0
        : 0;

      // Create period data from the new structure
      const periodData = [
        {
          period: 'Yesterday',
          members: classInfo.YesterdayCount?.toLocaleString() ?? '-',
          capital: (classInfo.YesterdayCapital / 100000)?.toFixed(2) ?? '-',
          borrowers: classInfo.YesterdayBor?.toLocaleString() ?? '-',
        },
        {
          period: 'Last Month',
          members: classInfo.LastMonthCount?.toLocaleString() ?? '-',
          capital: (classInfo.LastMonthCapital / 100000)?.toFixed(2) ?? '-',
          borrowers: classInfo.LastMonthBor?.toLocaleString() ?? '-',
        },
        {
          period: 'Last Year',
          members: classInfo.LastYearCount?.toLocaleString() ?? '-',
          capital: (classInfo.LastYearCapital / 100000)?.toFixed(2) ?? '-',
          borrowers: classInfo.LastYearBor?.toLocaleString() ?? '-',
        },
      ];

      console.log(`✅ Including ${classInfo.Class} in UI.`, {
        index: idx,
        members: memberCount,
        shareCapital: shareBal,
        periodData
      });
      ...
      return {
        ...
        color,
        members: memberCount,
        shareCapital: `₹ ${(shareBal / 100000).toFixed(2)}L`,
        periodData,
      };
    });
  };
