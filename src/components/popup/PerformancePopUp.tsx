...
//   trendData?: TrendData[]; // Add trend data to the main data structure
// }
...
// // Helper function to transform trend API data
// const transformTrendData = (apiData: any, tabId: TabType): TrendData[] => {
//   if (!apiData) return [];

//   // Determine which field contains the trend data based on tab type
//   let dataArray: any[] = [];
...
//   // Transform the data to our TrendData format
//   return dataArray.map((item: any) => {
//     // Determine the value based on tab type
//     let value = 0;
//     switch (tabId) {
//       case 'profit':
//       case 'fluid-resources':
...
//       case 'overdue':
//       case 'ldr':
//         value = item.ODPercentage || item.LDRPercentage || 0;
//         break;
//       default:
//         value = 0;
//     }
...
//     // Fetch trend data if it hasn't been loaded yet for this tab
...
//   // Generate trend chart data from API response
...
//     // Normalize the values for chart display (0-100 scale)
//     const values = currentTrendData.map(item => {
//       // For percentage-based metrics (overdue, ldr), use the percentage value directly
//       if (activeTab === 'overdue' || activeTab === 'ldr') {
//         return Math.min(Math.max(item.value, 0), 100); // Clamp between 0-100
//       }
      
//       // For amount-based metrics, normalize to a 0-100 scale
//       const allValues = currentTrendData.map(d => Math.abs(d.value));
//       const maxValue = Math.max(...allValues);
//       if (maxValue === 0) return 50; // Default middle value if no data
      
//       return (Math.abs(item.value) / maxValue) * 100;
//     });
...
//       case 'overdue':
//       case 'ldr':
//         value = item.Percentage || item.ODPercentage || item.LDRPercentage || 0;
//         break;
//       default:
//         value = 0;
//     }
...
//   // NEW STATE: Track the maximum absolute value for chart normalization
...
//       // Progress bar logic assumed to be fixed in external transformer
...
//       setActiveTab('profit');
//       setIsInitialLoad(true);
//       setTrendChartMaxValue(0); // Reset max value
//     }
//   }, [open]);
...
//   const handleTabChange = (tabId: TabType) => {
//     setActiveTab(tabId);
//     setTrendChartMaxValue(0); // Reset max value on tab change
...
//   // 🔥 FIX: useEffect to calculate max value when trend data changes
//   useEffect(() => {
//     if (currentTrendData && currentTrendData.length > 0) {
//       // Find the largest magnitude (absolute value) in the dataset.
//       const allValues = currentTrendData.map(d => Math.abs(d.value));
//       const maxValue = Math.max(...allValues, 1); // Use 1 to prevent division by zero
...
//   // 🔥 FIX: Updated logic to correctly scale for both amount and percentage metrics 
//   // IMPORTANT: This function no longer sets state. It is a pure calculation.
//   const generateTrendChartData = () => {
//     if (!currentTrendData || currentTrendData.length === 0) {
//       // Fallback to mock data if no trend data available
//       return [62, 66, 65, 62, 67, 67, 72];
...
//     // Use the state value that was calculated in useEffect
...
//       // Ensure the bar has a minimum visible height (e.g., 5%) if the value is non-zero but tiny.
...
//                                 opacity: index === generateTrendChartData().length - 1 ? 1 : 0.4, 
//                                 flex: 1,
//                                 borderTopLeftRadius: '8px',
//                                 borderTopRightRadius: '8px',
//                               }}
...
//     barChart: '#6DC1FF',
//     iconBackground: '#288AD3',
//     changePositive: '#298A42', // Adjusted color for fluid's positive change
//     changeNegative: '#EF4444',
//   },
//   'overdue': {
//     primary: '#F59E0B',
...
//     case 'working-capital':
//     case 'ldr':
//         // Mapping is generally consistent across amount/percentage fields
//       dataArray = apiData.OverDueDetails || apiData.WorkingCapital || apiData.LDRDetails || [];
//       break;
//     default:
//       return [];
//   }
...
//   // Fetch main tab data (remains the same)
//   const fetchTabData = useCallback(async (tabId: TabType) => {
//     setTabs(prev => ({
//       ...prev,
//       [tabId]: { ...prev[tabId], isLoading: true, error: null }
...
//   // Fetch trend data for a specific tab (remains the same)
//   const fetchTrendData = useCallback(async (tabId: TabType) => {
//     setTabs(prev => ({
//       ...prev,
//       [tabId]: { ...prev[tabId], isTrendLoading: true, trendError: null }
...
//   // Reset all tabs when popup closes (remains the same)
//   useEffect(() => {
//     if (!open) {
//       setTabs(getInitialTabsState());
//       setActiveTab('profit');
...
//   // Fetch all data when the popup opens for the first time (remains the same)
//   useEffect(() => {
//     if (open && isInitialLoad) {
//       setIsInitialLoad(false);
...
//   // 🔥 FIX 1: USE EFFECT to calculate max value when trend data changes (Stops infinite loop)
//   useEffect(() => {
//     if (currentTrendData && currentTrendData.length > 0) {
//       // Find the largest magnitude (absolute value) in the dataset.
//       const allValues = currentTrendData.map(d => Math.abs(d.value));
...
//   // FIX 2: REVISED to be a PURE CALCULATION function (Uses state, doesn't set state)
//   const generateTrendChartData = () => {
//     if (!currentTrendData || currentTrendData.length === 0) {
//       return [62, 66, 65, 62, 67, 67, 72];
//     }
...
//                             const isNegative = currentTrendData && currentTrendData[index] ? currentTrendData[index].value < 0 : false;
...
// // NEW HELPER: Formats the value for the Tooltip
...
//       setIsInitialLoad(true);
//       setTrendChartMaxValue(0);
//       setTooltipState(prev => ({ ...prev, show: false })); // Reset tooltip
//     }
//   }, [open]);
...
//     setActiveTab(tabId);
//     setTrendChartMaxValue(0);
//     setTooltipState(prev => ({ ...prev, show: false })); // Reset tooltip on tab change
...
//   // FIX 1: USE EFFECT to calculate max value when trend data changes
//   useEffect(() => {
//     if (currentTrendData && currentTrendData.length > 0) {
//       // Find the largest magnitude (absolute value) in the dataset.
//       const allValues = currentTrendData.map(d => Math.abs(d.value));
...
//   // FIX 2: REVISED to be a PURE CALCULATION function
//   const generateTrendChartData = () => {
//     if (!currentTrendData || currentTrendData.length === 0) {
...
//   // NEW COMPONENT: Individual Bar with Tooltip Logic
...
// =================  CHART WITH RED COLOR FOR NEGATIVE==============
import React, { useState, useEffect, useCallback, useRef } from 'react'; 
import {
  Box,
  Typography,
