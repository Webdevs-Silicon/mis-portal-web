...
const LoanDetailsPopup: React.FC<LoanDetailsPopupProps> = ({ onClose, open }) => {
  ...
  const fetchAllDataSequentially = async () => {
    ...
    try {
      console.group('🚀 Loan Popup Sequential Fetch');
      
      // 1️⃣ PERIOD WISE
      console.log('1. Fetching Period Wise data...');
      const periodData = await getLoanPeriodWise();
      setProgress(33);
      await delay(100); // Wait for token update
      
      // Update data immediately after first fetch
      setLoanData(prev => ({
        ...prev,
        Period: transformPeriodData(periodData)
      }));
      console.log('✅ Period Wise loaded');
      
      // 2️⃣ CATEGORY WISE
      console.log('2. Fetching Category Wise data...');
      const categoryData = await getLoanCategoryWise();
      setProgress(66);
      await delay(100);
      
      // Update data after second fetch
      setLoanData(prev => ({
        ...prev,
        Categories: transformCategoryData(categoryData)
      }));
      ...
      // 3️⃣ BRANCH WISE
      console.log('3. Fetching Branch Wise data...');
      const branchData = await getLoanBranchWise();
      setProgress(100);
      ...
      console.log('✅ Branch Wise loaded');
      
      setInitialLoad(true);
      console.log('🎉 All loan data loaded sequentially!');
      console.groupEnd();
      
    } catch (err: any) {
      console.error('❌ Error in sequential loan data fetch:', err);
